...
Pure module (no imports from dincli.cli) so both the SDK and the CLI can depend
on it without creating a circular dependency. Moved verbatim from
dincli.services.cid_utils as the first SDK clean-seed extraction (issue #20);
that path remains as a thin re-export shim for backward compatibility.
"""
import cid as _cid
