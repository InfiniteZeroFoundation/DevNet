...
///
/// Usage (CONTRACT must be one of: DinToken, DinCoordinator,
///        DinValidatorStake, DINModelRegistry):
///
///   CONTRACT=DinToken forge script foundry/script/UpgradePlatform.s.sol \
...
contract UpgradePlatform is Script {
    ...
    function run() external {
        ...
        address proxyAddress   = stdJson.readAddress(raw, string.concat(".", proxyKey));
    ...
    function _proxyKey(string memory contractName) internal pure returns (string memory) {
        if (keccak256(bytes(contractName)) == keccak256(bytes("DinToken"))) {
            return KEY_DIN_TOKEN;
        } else if (keccak256(bytes(contractName)) == keccak256(bytes("DinCoordinator"))) {
            return KEY_DIN_COORDINATOR;
        } else if (keccak256(bytes(contractName)) == keccak256(bytes("DinValidatorStake"))) {
            return KEY_DIN_VALIDATOR_STAKE;
        } else if (keccak256(bytes(contractName)) == keccak256(bytes("DINModelRegistry"))) {
