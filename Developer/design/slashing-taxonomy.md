...
`SlashingInvariants.t.sol` (Part 4b) was originally written to prove the pre-#65 "never moves tokens" behavior with a fuzz test rather than asserting it from reading the source once, which left 3 of its 12 tests failing against #65's burn/treasury split (`test_slash_doesNotTransferOrBurnTokens` and both balance/stranded-amount invariants). [PR #117](https://github.com/InfiniteZeroFoundation/DevNet/pull/117) realigned all three to assert the split instead, and added a fourth invariant (`invariant_totalAccountedStakeMatchesLedger`) covering an exact-ledger gap found while doing so — the file now carries 4 invariants + 9 fuzz/unit tests, all passing.
