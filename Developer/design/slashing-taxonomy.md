...
`DinValidatorStake.slash()` (`foundry/src/DinValidatorStake.sol:165`) does exactly one thing: it decrements `v.activeStake` (then `v.pendingWithdrawals` if active stake is insufficient) by the slashed amount and emits `ValidatorSlashed`. It never calls `DIN_TOKEN.transfer`, `burn`, or anything else that moves the underlying ERC20 balance. The tokens stay in the stake contract's own `balanceOf`, accounted to nobody. That's not a burn in the ERC20 sense — total supply is untouched — it's stranded accounting: gone from the validator's stake, unreachable by anyone, forever. `Developer/design/MECHANISM_DESIGN.md` §4 calls this "an accidental burn" — accurate in spirit (the validator can never get it back) but worth being precise about in code comments, since a real `burn()` call and "nobody can ever move this balance" have different security properties if `DinValidatorStake` is ever upgraded.

`SlashingInvariants.t.sol` (Part 4b) proves this with a fuzz test rather than asserting it from reading the source once.
